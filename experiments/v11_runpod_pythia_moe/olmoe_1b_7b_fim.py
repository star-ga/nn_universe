"""V11 — OLMoE-1B-7B FIM measurement (sparse-MoE substrate gap).

Adds the missing MoE point on the V11 FIM tier-hierarchy curve.
OLMoE-1B-7B-0924 (allenai) is a sparse Mixture-of-Experts language
model: ~1.3 B *active* parameters per token, ~6.9 B total parameters
(64 experts per layer; top-8 routing).

Schema (matches Pythia-6.9B output exactly):
    {
...
Comparison framing:
  Pythia-6.9 B is dense; OLMoE-1B-7B is the same total-parameter
  scale but sparsely activated. Whether the FIM tier hierarchy
  signature differs (Gini, T1/T3, eff_rank/n) under sparse routing
  is the V11 question. Inactive experts contribute zero gradient
  on a given probe — over 200 probes a non-trivial fraction of
  expert weights may never receive gradient flow, which itself
  is an FIM-spectrum signature worth measuring.

Memory plan: identical to pythia_6_9b_fim.py (model fp16 ≈14 GB,
FIM accumulators fp32 on CPU host ≈28 GB). Same A100-80GB pod
profile.
"""
    ...
    for i in range(n_probes):
        ...
        for n, p in net.named_parameters():
            if p.grad is not None and n in fim_cpu:
                ...
                probe_active += 1
        n_active_seen = max(n_active_seen, probe_active)
        if (i + 1) % 20 == 0:
            print(
                f"  probe {i+1}/{n_probes} (probe-active params: {probe_active})",
                flush=True,
            )
    for n in fim_cpu:
        fim_cpu[n] /= n_probes
...
def _streaming_stats_and_sample(fim_cpu, sample_size, seed):
    total_n = sum(v.numel() for v in fim_cpu.values())
    if total_n == 0:
        raise RuntimeError("FIM is empty.")
...
def _gini_on_positive(values: np.ndarray) -> float:
    ...
    iter-262 review fix: including exact-zero FIM entries from inactive
    MoE experts inflates Gini and breaks comparability with dense models
    like Pythia. Switch to ``values > 0`` to match top_1pct_mass and
    measure inequality on the active-parameter support.
    """
...
def main() -> int:
    ...
    ap.add_argument("--model", type=str, default="allenai/OLMoE-1B-7B-0924")
    ...
    ap.add_argument(
        ...
        help="MoE checkpoints often ship bf16 — keep that as default.",
    )
    args = ap.parse_args()
    ...
    # iter-262 extension: log hardware identity so paired-comparison
    # validity can be re-examined per seed. Cross-arch is generally
    # fine for FIM-diagonal accumulation but recording it makes the
    # question answerable post-hoc.
    arch_id = (
    ...
    payload = {
        ...
        "n_unique_texts": int(len(set(SAMPLE_TEXTS))),  # iter-262 fix coverage attestation
        "seed": int(args.seed),
        "T1T3": tier,
        "log10_T1T3": log10_t,
        "gini": g,
        ...
        "top_1pct_mass": tp1,
        "arch_id": arch_id,
        "spec_version": "v2-iter262",  # gini>0 + 200 texts + arch logged
    }
    out = Path(args.out)
