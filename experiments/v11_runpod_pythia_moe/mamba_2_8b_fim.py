"""V11.7 — Mamba-2.8B FIM measurement (SSM substrate at billion-param scale).

Closes the substrate_panel_coverage gap flagged by 3 of 3 V11.6 reviewers
(Mamba beyond V9.4 was conjectural). state-spaces/mamba-2.8b-hf is the
canonical SSM at the billion-param scale; same protocol as V9.10
Pythia-2.8B + V11.6 Pythia-6.9B / OLMoE.

Schema (matches Pythia-6.9B output verbatim):
    {
...
Significance: Mamba is selective-SSM (state-space model) substrate —
fundamentally different mechanism from attention-transformer (Pythia)
or sparse-MoE (OLMoE). Adding this datapoint extends the V11
substrate panel from 12 substrate classes to 13 with a true SSM at
billion-param scale. Combined with V11.6 Pythia-6.9B (dense attn)
and OLMoE-1B-7B (sparse-MoE), gives THREE distinct substrate families
at matched ~3-7B param scale for cross-substrate comparison.
"""
...
def main() -> int:
    ...
    ap.add_argument("--model", type=str, default="state-spaces/mamba-2.8b-hf")
    ...
    ap.add_argument(
        ...
        help="Mamba official checkpoints ship bf16; bf16 is canonical here.",
    )
    args = ap.parse_args()
