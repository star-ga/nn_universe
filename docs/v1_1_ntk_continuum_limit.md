...
**Q2 (V2.1): QEC spectral connection.** The FIM three-tier hierarchy observed in [1, §11.4] (Tier 1/Tier 3 ratio $637\times$) bears structural similarity to the spectral hierarchies observed in the weight matrices of trained quantum error correction (QEC) decoders, as noted in [1, §11.4] and the V2.1 roadmap. Whether the NTK spectrum of QEC-decoder networks follows the $\mathrm{SV} \sim N^\alpha$ power law, and whether the exponent $\alpha$ is universal across architectures, would provide evidence that the FIM hierarchy is a generic property of learned representations rather than a specific artifact of the cosmological toy model. This connection is deferred to V2.1.
