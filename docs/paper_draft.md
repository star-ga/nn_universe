...
- **GPT-Tiny pre-norm transformer** with causal masking, *tied input/output embeddings*, and synthetic next-token prediction (vocab 64, seq-len 16), depths 1 / 2 / 4 / 6 / 8 / 12, 3 seeds. Parameter range 59 k → 609 k. **This is the one architecture in our panel where the mechanism's predicted $\sqrt{L}$ direction does not hold**: the fit is statistically tight ($R^2 = 0.974$) but the slope is *negative* ($-0.22$), and the per-token tier ratio modestly *decreases* with depth from $T_1/T_3 \approx 450$ at $L{=}1$ to $\approx 250$ at $L{=}12$. The mechanism's universality claim is therefore **narrowed** by this finding, not preserved by it: the Hanin–Nica log-normal does not hold under tied input/output embeddings + softmax-attention coupling, where the embedding parameters dominate the FIM tier-1 mass and dilute (rather than amplify) as more transformer blocks are added. Three implications: (i) the universality claim must be *scoped* to architectures without tied embeddings dominating the FIM mass; (ii) untying input/output embeddings (GPT-3-style scaling) is a future-work test of whether the negative slope flips; (iii) ResNets remain the strongest large-depth confirmation of the positive direction.
