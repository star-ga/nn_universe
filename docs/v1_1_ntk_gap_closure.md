...
**Sign-fix note (audit v2, 2026-04-23):** the original wording of this section stated the finite-width correction contributes a *positive* shift to the measured slope. model-C v2 flagged this as a sign error. The derivation below corrects it: the shift is *negative* in the asymptotic regime, and the mechanism that can generate a positive slope bias at finite $N$ is distinct — it is the feature-learning leakage of Section 2.2, not the NTK finite-width correction.
