...
**STARGA, Inc.**
**Nikolai Nedovodin, 2026-04-26 (V11 revision)**
...
| 7 | Production-scale checkpoints (ResNet-50 V1+V2, ViT-L/16, GPT-2-medium) sit firmly in the deep-sequential band | **Empirically supported, multi-seed CIs reported** | V9.5–V9.7 + multi-seed `v9_multi_seed_production_results.json` |
...
**Checkpoint provenance and data exposure.** All production-scale measurements (§4.6) use the canonical *published* pretrained weights from torchvision and HuggingFace (ResNet-50 V1+V2, ViT-L/16 SWAG-LINEAR, GPT-2-medium). We do **not** access the raw ImageNet-1K or WebText training data; the FIM-diagonal probes are drawn from synthetic ImageNet-statistics-normalised Gaussian inputs (vision) and a fixed 12-sentence English text sample (language). The published weights themselves are the only product of the original training data that enters our analysis. This avoids reopening the data-provenance debate around ImageNet-1K and WebText (both of which have known dataset-curation concerns documented elsewhere); our work is downstream-of-checkpoint and does not amplify those concerns.
