...
A large-$N$ tier-fraction theorem (formal proof that the limit fractions $(0.01, 0.49, 0.50)$ are well-defined under a specified parameter-prior class) and a tighter mechanism for residual + attention architectures are left open for future work. **Three distinct from-scratch trajectory patterns are documented in §4.6**: CIFAR-10 ResNet-18 monotonic *decrease*; Imagenette ResNet-50 *increase* with epoch-1 transient peak; and Imagenette ViT-B/16 jump-from-below-threshold (random init $T_1/T_3 = 0.37$, epoch 1 jumps to $> 10^5$, epoch 10 settles at $2.9 \times 10^4$). The dichotomy magnitude is preserved across all three patterns at every trained checkpoint; the *direction* of trajectory evolution is architecture/dataset-specific. Cosmological framings of the deep-layered-sequential class (Vanchurin 2020; Nedovodin 2026) are out of scope for this paper and discussed only briefly in Appendix C.
