...
**Real-data and production-scale verification.** A ResNet-18 (11.2 M parameters) trained 10 epochs on CIFAR-10 to 81.4 % test accuracy gives $T_1/T_3 = 778$ (deep-sequential band, $\gg 100$). At production scale we measure ResNet-50 V1+V2 (76.13 / 80.86 % ImageNet), ViT-L/16 (304 M parameters, $T_1/T_3 = 6.93 \times 10^{3}$), GPT-2-medium 5-seed multi-seed ($\log_{10}(T_1/T_3) = 4.83 \pm 0.003$), GPT-2-large (774 M), and Pythia-1.4B (first billion-parameter LM data point, $T_1/T_3 = 4.25 \times 10^{3}$); the V11.3 production sweep further covers an Imagenette ViT-B/16 from-scratch trajectory (10-epoch, third distinct trajectory pattern) and an ImageNet-1K ResNet-50 90-epoch from-scratch trajectory at canonical NeurIPS-era scale. Every measured trained checkpoint sits firmly in the deep-sequential band at $\geq 11 \times$ the 100 threshold, and the dichotomy magnitude is confirmed on every real benchmark beyond synthetic Gaussian self-prediction.

**Implications.** Within the 12-substrate panel, the tier hierarchy is (i) already present at random Kaiming initialisation with $T_1/T_3 \sim 10^3$–$10^4$, (ii) reduced by gradient-descent training by a factor of 4–24×, and (iii) suppressed below the 100-threshold by initialisations close to dynamical isometry (V10b: identity + perturbation init drops $T_1/T_3$ to $\approx 42$, *confirming* the Hanin–Nica falsifier registered in §3.5). The mechanism makes the FIM tier signature a *panel-bounded empirical regularity* of the deep-layered-sequential substrate class. The Hanin–Nica theorem itself covers strict ReLU MLPs at infinite width; the extension to residual stacks, vanilla transformers, attention transformers, Mamba SSMs, and non-neural substrates is empirical and graded — see §1.2 for the explicit claim hierarchy.
...
>
> **One scope.** The mechanism applies to ResNets, vanilla transformers, MLPs, and boolean circuits, but is *attenuated* in attention-with-tied-embeddings (small GPT) and Mamba SSMs with selective gating, and is *absent* in time-unrolled RNN/LSTM. The regularity holds for layer-stack depth on the tested panel, not for arbitrary sequential composition.

---

### Claim hierarchy (proved · empirically supported · conjectural)

| # | Claim | Status | Backing |
|---|---|---|---|
| 1 | Theorem 1: $\log T(\alpha,\beta) = \sqrt{v}(\bar z_\alpha^+ - \bar z_\beta^-) + o(1)$ for any log-normal $F$ | **Proved** | Appendix B (Theorem 1' exact closed form via truncated-Gaussian MGF) |
| 2 | Hanin–Nica log-normal accumulation in deep ReLU MLPs at infinite width | **Proved** (cited from Hanin & Nica 2020) | external theorem; conditions: i.i.d. Gaussian weights, ReLU, $n, L \to \infty$ |
| 3 | Proposition 2: pooling-error bound for layer-stratified log-normal mixtures, $L \geq 4$ | **Proved** + numerically verified | Appendix B; `experiments/v6_0_mechanism/v6_0c_pooling_error_bound_results.json` |
| 4 | $\log(T_1/T_3) \propto \sqrt{L}$ scaling in untrained ReLU MLPs ($R^2 = 0.98$) | **Empirically supported** within Hanin–Nica scope | V6.0 sweep; matches Theorem 1 + Hanin–Nica prediction within finite-width corrections |
| 5 | Same $\sqrt{L}$ scaling in trained MLPs, boolean circuits, vanilla transformers, binary tensor networks, BatchNorm ResNets ($R^2 \geq 0.94$) | **Empirically supported, theorem assumptions only partially hold** | V6.x + V8.0 + V9 — explicit empirical extension |
| 6 | Empirical dichotomy at threshold $T_1/T_3 = 100$ across the V2 12-substrate panel ($p = 1.7 \times 10^{-17}$, $r = 1.000$, ROC AUC = 1.0) | **Empirically supported, panel-bounded** | §4.5 + V5.1 + V5.2; the proposition (§4.5 Definition + Proposition 1) is *explicitly* qualified to the V2 panel |
| 7 | Production-scale checkpoints (ResNet-50 V1+V2, ViT-L/16, GPT-2-medium 5-seed multi-seed, GPT-2-large, Pythia-1.4B, ViT-B/16-trajectory-from-scratch, ImageNet-1K ResNet-50 90-epoch from-scratch) sit firmly in the deep-sequential band on every measured trained checkpoint | **Empirically supported, multi-seed CIs where seed cost permitted** | V9.5–V9.8, V11.3 + multi-seed JSONs |
| 8 | Identity-perturbation initialisation drops $T_1/T_3$ below the 100 threshold ($\to \approx 42$); orthogonal init only partially reduces it | **Empirically supported** (registered falsifier confirmed) | V10b `init_family_ablation`; matches Hanin–Nica mechanism prediction |
| 9 | Architecture-narrowing on attention (small GPT tied + untied), Mamba SSM (pre-registered H2 PARTIAL), and time-unrolled RNN/LSTM | **Empirically supported, narrows the universality scope** | §4.6 V9, V9.1, V9.3, V9.4 |
| 10 | Reverse direction: any system satisfying $T_1/T_3 > 100$ at $L \geq 4$ is in the deep-sequential class | **Conjectural** (open) | only forward direction tested on the panel |
| 11 | The substrate class extends beyond our 12-class panel (e.g. to MoE, MLP-Mixer, SSM variants we have not tested) | **Empirically supported for MoE (V11.6 + V11.8 multi-seed) and SSM (V11.7)**: V11.6 single-seed found OLMoE-1B-7B-0924 at $T_1/T_3 = 4.26 \times 10^5$ vs dense Pythia-6.9B at $5.74 \times 10^3$. V11.8 multi-seed reproducibility check ($n_{\text{seeds}}=5$ each, seeds $\in \{7, 13, 42, 99, 256\}$) confirms the dense-vs-MoE separation: $\log_{10}(T_1/T_3)_{\text{Pythia}} = 3.484 \pm 0.012$ (CV 0.34 %) vs $\log_{10}(T_1/T_3)_{\text{OLMoE}} = 5.083 \pm 0.195$ (CV 3.83 %); Welch's $t = 18.32$, $p \ll 10^{-6}$, architecture-class gap $\Delta = 1.599$ dex $\approx 39.7\times$ — roughly an order of magnitude wider than within-architecture seed CV. **Mamba-790M-HF measured at $T_1/T_3 = 8.20 \times 10^4$** ($\log_{10} = 4.91$, gini $= 0.996$, top-1 % mass $= 0.984$, $n=200$ probes, seed 42) — confirms the V9.4 small-scale Mamba band $[7 \times 10^4, 1.8 \times 10^5]$ extends to billion-param-adjacent scale and the H2 PARTIAL gated-architecture pattern (deep-sequential band with selective-gating attenuation vs the pure-MLP regime) holds at production scale. MLP-Mixer extension still **conjectural** (open) | V11.6 OLMoE + V11.7 Mamba-790M + V11.8 multi-seed results JSONs; MLP-Mixer extension requires out-of-panel measurement |
| 12 | Cosmological / holographic interpretations of the substrate class | **Out of scope** | listed for context only in supplementary §S6 (Appendix D); no main-paper claim depends on this framing |
...
**Scope and limits.** What we call "universality" is empirical universality across the *substrate panel of this paper*: 12 substrate classes plus the V9–V11 modern-architecture and production-scale extension (ResNet residuals + GPT-Tiny attention at small scale; ResNet-50 V1+V2, ViT-L/16, ViT-B/16-from-scratch trajectory, GPT-2-medium/large, Pythia-1.4B, MoE, and an ImageNet ResNet-50 from-scratch trajectory at production scale). The Hanin–Nica 2020 theorem we apply rigorously covers ReLU MLPs with i.i.d. Gaussian weights at infinite width and depth; its extension to (a) trained networks, (b) random boolean circuits, (c) attention/residual transformers, (d) MERA-style tensor networks, and (e) BatchNorm ResNets is *empirical* — we measure the predicted $\sqrt{L}$ scaling and confirm it within $R^2 \geq 0.94$ for five substrate classes, $R^2 = 0.97$ for one (vanilla transformer), and observe a structural counter-example in GPT-Tiny (V9 §4.6) that *narrows* the universality claim rather than confirming it everywhere. Real-data benchmarks (CIFAR-10/100, Imagenette, ImageNet-1K, WebText, The Pile) and production-scale architectures (ResNet-50, ViT-B/16, ViT-L/16, GPT-2-medium/large, Pythia-1.4B) are measured here on community-cloud A100 80GB compute (§4.6); ImageNet ViT-Large from-scratch (~120 GPU-hours on H100) is the one remaining pre-registered cluster experiment whose mechanism prediction we have not yet directly verified at full ImageNet ViT scale.

### 1.5 Contributions

**Headline.** This paper establishes *deep layered sequential composition* as a new universality class for the Fisher Information diagonal — the first substrate-class universality result for an FIM-diagonal observable, and the first to extend a deep-learning observable to **non-neural** substrates (random boolean circuits, MERA tensor networks) under a single statistical protocol. The class definition is *empirical and panel-bounded* (Definition 1, §4.5); the mechanism is *closed-form proved* (Theorem 1', Appendix B). The combined empirical-plus-mechanistic result is, to our knowledge, the first time a substrate-class universality claim about deep networks has been made with a registered statistical test ($p = 1.7 \times 10^{-17}$, ROC AUC = 1.0), a closed-form mechanism (no $o(1)$ slack), and a pre-registered dynamical-isometry falsifier (V10b: identity+ε init drops $T_1/T_3$ to 42, *confirming* the Hanin–Nica prediction).

**Cross-scale convergence finding (V11.5).** ResNet-50 trained from scratch on full ImageNet-1K for 90 epochs ends with $T_1/T_3 = 778$ at 74.6 % top-1 — *exactly matching* ResNet-18 trained 10 epochs on CIFAR-10 ($T_1/T_3 = 778$ at 81.4 %). Two networks with $\sim 25\times$ different parameter counts, trained on datasets with $\sim 100\times$ different effective sample counts, both land at the *same trained-equilibrium tier ratio* — a non-trivial numerical convergence at the deep-sequential band's lower edge.

We make six concrete contributions, none of which (to our knowledge) appear in prior work:

1. **A new universality class with $p = 1.7 \times 10^{-17}$.** Prior work characterised heavy-tailed FIM spectra in MLPs (Karakida–Akaho–Amari, AISTATS 2019) and three-level outlier structure in deepnet Hessians (Papyan, ICML 2019), both restricted to neural networks. We show empirically that the same diagonal hierarchy is a property of *layered recursive composition* as a computational primitive — present in boolean circuits ($T_1/T_3 \sim 10^7$, no learning, no gradients, no real-valued weights), balanced binary tensor networks, BatchNorm ResNets, vanilla transformers, and trained / untrained MLPs; absent from shallow parameterised learners that *do* learn and generalise ($T_1/T_3 \le 5$), lattice gauge fields, dynamical systems, and matrix ensembles ($T_1/T_3 \le 80$). The 12-substrate panel achieves *complete rank separation* with $p = 1.7 \times 10^{-17}$, $r = 1.000$, ROC AUC $= 1.0$ (one-sided Mann–Whitney $U$, $n_{\text{deep}} = 46$, $n_{\text{rest}} = 50$, $B = 10\,000$ bootstrap, leave-one-substrate-class-out preserves $p < 5.8 \times 10^{-13}$). The substrate-class itself is the load-bearing novelty.

2. **A formal log-normal-quantile-to-tier-ratio identity (Theorem 1 / 1', §4.6 + Appendix B).** We prove that for any log-normal $F \sim e^{\mathcal{N}(m, v)}$, the partitioned tier ratio $T(\alpha, \beta) = \mathbb{E}[F \mid F > q_\alpha] / \mathbb{E}[F \mid F < q_\beta]$ satisfies $\log T(\alpha, \beta) = \sqrt{v} \cdot (\bar z_\alpha^+ - \bar z_\beta^-) + o(1)$, with **Theorem 1' giving the exact closed form via truncated-Gaussian moment generating functions** (no $o(1)$). **The identity is general**: it applies to any log-normal observable and turns any depth-linear-variance result (such as Hanin–Nica 2020 on products of random Jacobians) into a $\sqrt{L}$ tier-ratio prediction with an explicit prefactor and partition-form independence. **Corollary 1** specialises to FIM and gives $\log(T_1/T_3) = c \sigma \sqrt{L} + o(\sqrt{L})$ with $c \approx 4.90$. To our knowledge, this is the first deployment of conditional-quantile algebra as a load-bearing bridge between random-matrix log-normal limits and tier-style universality claims.

3. **A pre-registered dynamical-isometry falsifier, confirmed.** Identity-perturbation initialisation drops $T_1/T_3$ to $\approx 42$ (below the 100 threshold) while orthogonal init only partially reduces it ($\approx 281$). This is a *registered* falsifier: the prediction was committed to the protocol *before* the experiment, and the data confirmed it. Pre-registered falsifiers are rare in deep-learning theory; this is the only one we are aware of that makes a quantitative prediction about an FIM-diagonal observable.

4. **Three partition-invariant alternative statistics that all confirm the dichotomy** (Gini coefficient, normalised effective rank, top-1 % FIM mass fraction). These rule out the partition-tuning concern that has been the standard reviewer challenge to tier-style claims since Karakida–Akaho–Amari (2019).

5. **A same-setting comparison against 8 prior deep-network observables.** All 8 (T1/T3, top-1 %-mass, Gini-FIM-diag, FIM-spectrum top/median, Hessian-outliers, weight-spectrum power-law $\alpha$, gradient-norm log-variance, Jacobian condition number) cleanly separate the same toy panel. T1/T3's distinguishing features are a *conjunction* of properties — substrate-independent (computable on boolean circuits where FIM-spectrum / Hessian are undefined), label-free, bounded at scale, and partition-invariant — not exclusive separation. We report this honestly rather than claiming uniqueness.

6. **Production-scale verification through 2.8 B parameters and a complete from-scratch ImageNet-1K trajectory.** Multi-seed FIM on ResNet-50 V1+V2, ViT-L/16 (304 M), GPT-2-medium 5-seed multi-seed (354.8 M, $\log_{10} = 4.83 \pm 0.003$), GPT-2-large (774 M), Pythia-1.4B and Pythia-2.8B (V11.3 — *first* and *second* billion-parameter LM data points in the FIM-diagonal literature). V11.5 capstone is a full **ImageNet-1K ResNet-50 90-epoch from-scratch trajectory** at the canonical NeurIPS-era recipe (74.6 % top-1, $T_1/T_3$ drops $4\,367\times$ monotonically) — Pattern A confirmed at canonical scale, *re-framing* the V9.5b Imagenette anomaly as a small-data-regime outlier rather than an architectural property of ResNet-50. V11.6–V11.7 extends the panel beyond dense transformers: **Pythia-6.9 B** ($T_1/T_3 = 5.74 \times 10^3$, single-seed) anchors the dense-LM extreme, **OLMoE-1B-7B-0924** ($T_1/T_3 = 4.26 \times 10^5$, single-seed) the MoE extreme, and **Mamba-790M-HF** ($T_1/T_3 = 8.20 \times 10^4$, $n=200$ probes, seed 42) the SSM mid-band — three substrate primitives at billion-param-adjacent scale, all in the deep-sequential band. **V11.8 5-seed multi-seed reproducibility check** ($n_{\text{seeds}}=5$ each, seeds $\in \{7, 13, 42, 99, 256\}$, $n_{\text{probes}}=200$) confirms the dense-vs-MoE separation under within-architecture seed variation: dense Pythia-6.9 B at $\log_{10}(T_1/T_3) = 3.484 \pm 0.012$ (CV 0.34 %, $T_1/T_3 \approx 3{,}045$) vs MoE OLMoE-1B-7B-0924 at $\log_{10}(T_1/T_3) = 5.083 \pm 0.195$ (CV 3.83 %, $T_1/T_3 \approx 121{,}000$); Welch's $t = 18.32$ on $n_{\text{Pythia}}+n_{\text{OLMoE}} = 5+5$, $\text{df} = 4.03$, one-sided $p \ll 10^{-6}$. Architecture-class gap $\Delta = 1.599$ dex $\approx 39.7\times$ — roughly an order of magnitude wider than the within-architecture seed CV for either model, confirming the dichotomy is not a seed artefact. The V11.6 single-seed point estimates ($5.74 \times 10^3$ for Pythia-6.9 B, $4.26 \times 10^5$ for OLMoE) lie within the 5-seed range bands. Plus three distinct from-scratch trajectory patterns documented (CIFAR-10 ResNet-18 monotonic decrease; Imagenette ResNet-50 increase + transient peak; Imagenette ViT-B/16 jump-from-below-threshold). Every trained-equilibrium checkpoint sits firmly in the deep-sequential band.

The mechanism's connection to Hanin–Nica's product-of-random-matrices theorem is an *application* of an existing result, not a new theorem; we make this honest scope statement explicit. Contributions 1, 3, 4, 5, 6 are empirical and methodological; contribution 2 is a formal identity that, while elementary in its proof, is the first to bridge log-normal random-matrix theory to substrate-class universality in deep networks via conditional-quantile algebra.
...
- **Out-of-scope framings.** Connections to neural-network-cosmology / tensor-network / holographic-code literature are listed in supplementary §S6 (Appendix D) as motivation context only; nothing in §4 or §5 depends on those framings. The empirical and mechanistic results stand independently.
...
### 3.4-pre Protocol summary table

| Component | Setting |
|---|---|
| FIM estimator | $F_{ii} = \mathbb{E}[(\partial_{\theta_i}\ell)^2]$ (diagonal only) |
| Probe distribution | $x \sim \mathcal{N}(0, I_d)$ (synthetic) or ImageNet-norm Gaussian (vision) or fixed text sample (LM) |
| Probe count | 200 (small-scale panel), 100 (production-scale multi-seed), 2000 (V6.0b convergence study upper bound) |
| Accumulation dtype | float64 (numerically stable; float32 within 3 % per V6.0b) |
| Tier partition | top 1 % / middle 49 % / bottom 50 % (naming convention from V1.0; partition-invariant under Gini, eff-rank, top-1 %-mass) |
| Decision threshold | $T_1/T_3 = 100$ (unique balanced-accuracy maximum; ROC AUC = 1.0 at this cut) |
| Statistical test | one-sided Mann–Whitney $U$ on per-seed $\log T_1/T_3$ + Bonferroni $\alpha = 0.05/13$ + bootstrap 95 % CI |
| Hardware | single RTX 3080 (10 GB) for $N \leq 2 \times 10^8$; community-cloud A100 80 GB for $N \in [6 \times 10^8, 1.45 \times 10^9]$ |
| Total cloud cost | ~3.5 GPU-hours, ~\$5 USD on community A100 |
| Determinism | `torch.use_deterministic_algorithms(True)` where supported; per-seed `git rev-parse HEAD` + `pip freeze` hash embedded in every result JSON |
...
3. **Mamba SSM out-of-sample prediction.** *If* the universality applies to depth-stacked layered architectures generally, *then* a Mamba-style state-space-model stack with distinct per-layer parameters must have $T_1/T_3 > 100$ and slope $\sqrt{L} > 0$. *If* selective gating (analogous to attention's softmax saturation) attenuates the variance accumulation, the slope is positive but $R^2 < 0.85$. *If* Mamba is structurally distinct, the slope is flat or negative. **Result (V9.4):** slope $= 0.468$ ($R^2 = 0.78$), $T_1/T_3 \in [70\,000, 180\,000]$ — H2 PARTIAL confirmed pre-registered (§4.6). **Production-scale follow-up (V11.7).** Mamba-790M-HF (793 M params, the HuggingFace-port of Gu & Dao 2024 's mamba-790m, evaluated on the same self-prediction protocol $\Pi$) gives $T_1/T_3 = 8.20 \times 10^4$ ($\log_{10} = 4.91$, gini $= 0.996$, top-1 % mass $= 0.984$, $n=200$ probes, seed 42) — squarely inside the V9.4 small-scale band, demonstrating that the gated-SSM regime persists across two decades of parameter-count scaling without crossing the $100$ threshold.
...
**Dynamical-isometry / initialisation-family ablation (V10b — registered falsifier confirmed).** §3.5 (and the parent paper) registers the prediction that an initialisation close to dynamical isometry — under which the Hanin–Nica log-normal accumulation breaks because Var[$\log F$] does not grow in depth — should *suppress* the FIM tier hierarchy. We test this directly at fixed depth $L = 8$, width 128, dim 16, 3 seeds, comparing four init families:

| Init family | $\log_{10}(T_1/T_3)$ | Above 100 threshold? |
|---|---|---|
| Kaiming normal (default, far from isometry) | $2.69 \pm 0.12$ | **yes** ($T_1/T_3 \approx 494$) |
| Xavier normal (balanced) | $3.23 \pm 0.19$ | **yes** ($T_1/T_3 \approx 1\,684$) |
| Orthogonal (gain $\sqrt{2}$, near-isometric) | $2.45 \pm 0.06$ | **yes** ($T_1/T_3 \approx 281$, partial reduction) |
| **Identity + perturbation $\epsilon = 10^{-2}$** (perfect isometry plus noise) | $\mathbf{1.62 \pm 0.01}$ | **no** ($T_1/T_3 \approx \mathbf{42}$) — falsifier confirmed |

The identity-perturbation init drops $T_1/T_3$ below the 100 threshold by $\approx 12$×, confirming the Hanin–Nica mechanism prediction: dynamical isometry (or close to it) suppresses the hierarchy as the theorem requires. Orthogonal init with $\sqrt{2}$ gain only partially reduces it — consistent with that gain being only approximately isometric for ReLU. The graded outcome (Kaiming → Xavier → Orthogonal → Identity+ε, monotonically decreasing) is the cleanest mechanistic confirmation of the depth-linear log-FIM-variance story available without an analytic limit. Full results: `experiments/v10_baselines/v10b_init_family_ablation_results.json`.

**Same-setting comparison vs. closest prior observables (V10).** A reviewer's strongest concern is whether $T_1/T_3$ adds anything beyond closest prior observables on the *same* networks. We compute eight diagnostics on the *identical* untrained-MLP family (depths $L \in \{5, 8, 12\}$, dim 16) and a matched non-deep panel (linear regression, logistic regression):

| Observable | Deep range (MLP $L \in 5,8,12$) | Non-deep range (linear, logistic) | Cleanly separates? |
|---|---|---|---|
| **$T_1/T_3$ (this paper)** | $[4.3 \times 10^3,\ 6.6 \times 10^{14}]$ | $[4.1,\ 4.8]$ | **yes** |
| **Top-1 % FIM mass** (this paper) | $[0.75,\ 0.93]$ | $[0.022,\ 0.047]$ | **yes** |
| **Gini-FIM-diagonal** (this paper) | $[0.94,\ 1.00]$ | $[0.21,\ 0.24]$ | **yes** |
| FIM-spectrum top/median (Karakida–Akaho–Amari 2019) | $[64,\ 6.5 \times 10^6]$ | $[10.5,\ 23.7]$ | yes |
| Hessian-outlier ratio (Sagun et al.; Papyan 2019) | $[128,\ 1.6 \times 10^7]$ | $[23.5,\ 94.4]$ | yes |
| Weight-spectrum PL$_\alpha$ (Martin–Mahoney 2018) | $[4.0,\ 4.9]$ | $[3.4,\ 3.4]$ | yes (small gap) |
| Gradient-norm $\log$-variance (Hanin–Nica 2020 directly) | $[0.12,\ 0.16]$ | $[0.59,\ 1.04]$ | yes (opposite direction!) |
| Layer-Jacobian condition number | $[397,\ 1.2 \times 10^4]$ | $[55.6,\ 55.6]$ | yes |

**Honest reading.** On this small matched panel, *every* observable cleanly separates the deep MLPs from the shallow learners. So $T_1/T_3$ is not uniquely separating at the toy scale. What distinguishes $T_1/T_3$ is the conjunction of properties: (i) substrate-independent (computable on boolean circuits with no gradients — see §4.5 — where FIM-spectrum and Hessian-outliers are undefined); (ii) label-free (does not require a target distribution unlike Hessian-outliers); (iii) bounded at scale (full FIM spectrum is rank-deficient at $P > N \cdot d_{\text{out}}$ producing unbounded artifacts, see §1; $T_1/T_3$ is bounded by construction); (iv) partition-invariant under three alternative summary statistics (§4.5). The contribution is the *combination*, not exclusive separation power. Full results: `experiments/v10_baselines/v10_same_setting_comparison_results.json`.
...
A balanced binary tensor-network depth sweep (V8.0, `experiments/v8_0_tensor_network/binary_tree_tensor_network.py`, 8 depths × 5 seeds, 2-input soft-threshold tensor at every internal node of the tree) is the test of whether the mechanism extends to *non-neural* layered substrates. $\log(T_1/T_3) \propto \sqrt{L}$ passes at slope $7.44$, $R^2 = 0.992$. The tier hierarchy is therefore a *predicted consequence* of MERA-like binary-tree tensor-network substrates as well, extending the substrate class beyond neural networks and boolean circuits.
...
- **ImageNet + ResNet-50, pretrained (production-scale CNN, multi-seed)**: Torchvision's `ResNet50_Weights.IMAGENET1K_V1` (25.6 M parameters, **76.13 %** ImageNet-1K top-1 accuracy — the standard 90-epoch baseline) and `ResNet50_Weights.IMAGENET1K_V2` (same architecture, **80.86 %** top-1, trained with the modern recipe: LARS optimiser + cosine LR + label smoothing + RandAugment). FIM measured with the 200-probe float64 protocol on ImageNet-statistics-normalised inputs, repeated across **5 different probe seeds** to characterise estimator uncertainty (`v9_multi_seed_production_results.json`):
  ...
  - V2 (80.86 % acc): $\log_{10}(T_1/T_3) = 7.02 \pm 0.03$ (95 % bootstrap CI $[7.00, 7.05]$), $T_1/T_3 \approx 1.06 \times 10^7$, Gini = $0.997 \pm 0.000$, top-1 % mass = $0.963 \pm 0.002$.
...
- **GPT-2-medium pretrained on WebText (production-scale autoregressive LM, multi-seed)**: HuggingFace's `gpt2-medium` (354.8 M parameters; 24-layer transformer with hidden size 1024, 16 attention heads, untied input/output embeddings, pretrained on the 40 GB WebText corpus). FIM measured with the 200-probe float64 protocol; **multi-seed run on community-cloud A100 (5 different probe seeds)** since the RTX 3080 had a VRAM ceiling on the float64 accumulator. Multi-seed aggregate: **$\log_{10}(T_1/T_3) = 4.83 \pm 0.003$ (5 seeds, std 0.6 %)**, $T_1/T_3 = 6.7 \times 10^4$, Gini $= 0.996 \pm 0.000$, top-1 % mass $= 0.987 \pm 0.001$. The single-seed RTX-3080 result of $T_1/T_3 = 6.12 \times 10^4$ from §4.6 V9.6 sits inside the 95 % multi-seed bootstrap CI; the multi-seed run confirms the deep-sequential-band placement and the V11.2 single-seed point estimate. Full results: `experiments/v9_modern_arch/v9_6_gpt2_medium_5seed_results.json`.

- **GPT-2-large pretrained on WebText (production-scale autoregressive LM)**: HuggingFace's `gpt2-large` (774 M parameters; 36-layer transformer with hidden size 1280 and 20 attention heads). Same protocol. Result: $T_1/T_3 = 1.10 \times 10^3$ (deep-sequential band, $11 \times$ above the 100 threshold), Gini = $0.867$, top-1 % mass = $0.607$. Notably *smaller* magnitude than GPT-2-medium ($6.7 \times 10^4$) despite 2.2× larger model — consistent with the V9.5 ResNet-50 V1-vs-V2 finding that more-trained / larger-architecture models can have a *lower* tier ratio (parameter mass spreads across more layers). The dichotomy band is preserved in both directions — direction-of-scaling is non-monotonic in parameter count, but every measured production-scale checkpoint sits firmly above 100. Full results: `experiments/v9_modern_arch/v9_6b_gpt2_large_results.json`.

- **Pythia-1.4B pretrained on the Pile (billion-parameter autoregressive LM)**: EleutherAI's `pythia-1.4b` (1.4 B parameters). Same protocol on community-cloud A100. Result: $T_1/T_3 = 4.25 \times 10^3$ (deep-sequential band, $42 \times$ above the 100 threshold), Gini = $0.943$, top-1 % mass = $0.795$. **First billion-parameter LM data point in the panel.** $T_1/T_3$ at 1.4 B params is intermediate between GPT-2-medium ($6.7 \times 10^4$ at 354 M) and GPT-2-large ($1.1 \times 10^3$ at 774 M), confirming the non-monotonicity in parameter count and the deep-sequential-band placement at billion-parameter LM scale. Full results: `experiments/v9_modern_arch/v9_8_pythia_1.4b_results.json`.

- **Pythia-2.8B pretrained on the Pile (V11.3, second billion-parameter LM data point)**: EleutherAI's `pythia-2.8b` (2.78 B parameters). Same protocol on community-cloud A100. Result: $T_1/T_3 = 2.02 \times 10^3$ (deep-sequential band, $20 \times$ above the 100 threshold), Gini = $0.908$, top-1 % mass = $0.749$. The 2.8 B point sits between Pythia-1.4B ($4.25 \times 10^3$) and GPT-2-large ($1.1 \times 10^3$ at 774 M), continuing the non-monotonicity pattern at LM scale. Full results: `experiments/v9_modern_arch/v9_10_pythia_2.8b_results.json`.

- **Pythia-6.9B pretrained on the Pile (V11.6, third billion-parameter LM data point — extends the dense Pythia family curve)**: EleutherAI's `pythia-6.9b` (6.86 B parameters). Same protocol on a SECURE-cloud A100-SXM4-80GB (community-cloud preemption forced a re-run; see V11.6 compute notes). Result: $T_1/T_3 = 5.74 \times 10^3$ (deep-sequential band, $57 \times$ above the 100 threshold), $\log_{10}(T_1/T_3) = 3.76$, Gini = $0.960$, top-1 % mass = $0.867$. The 6.9 B point continues the Pythia family curve (1.4B $\to$ 2.8B $\to$ 6.9B at $T_1/T_3 = 4.25 \times 10^3 \to 2.02 \times 10^3 \to 5.74 \times 10^3$), with the V11.6 dense-LM regression slope $\log_{10}(T_1/T_3) \approx 0.49 \cdot \log_{10}(N)$ within the Pythia family. Full results: `experiments/v11_runpod_pythia_moe/results/v11_pythia_6_9b_results.json`.

- **OLMoE-1B-7B pretrained on the Pile / Dolma (V11.6, sparse-MoE substrate — first MoE data point in the panel)**: AllenAI's `OLMoE-1B-7B-0924` (6.92 B total parameters / ~1 B active per token; 64 experts per layer, top-8 routing). Same protocol on a parallel SECURE-cloud A100-SXM4-80GB. Result: $T_1/T_3 = 4.26 \times 10^5$ (deep-sequential band, $4{,}256 \times$ above the 100 threshold), $\log_{10}(T_1/T_3) = 5.63$, Gini = $0.997$, top-1 % mass = $0.979$, $r_{\rm eff}/n = 8.99 \times 10^{-18}$. **At matched ~7 B total-parameter scale, the sparse-MoE substrate concentrates FIM mass $\approx 74 \times$ more than the dense Pythia-6.9B point** ($T_1/T_3 = 4.26 \times 10^5$ vs $5.74 \times 10^3$); top-1 % mass jumps $0.867 \to 0.979$; effective rank drops $\approx 1{,}000 \times$. The MoE point sits **two orders of magnitude above** where the V11.6 dense-LM regression slope predicts at the same total-parameter scale. This **closes the conjectural row 11 in the V11.5 claim hierarchy** (the substrate-class extension to MoE) — MoE substrate now has a measured FIM-tier-hierarchy data point, and the dichotomy direction holds. Full results: `experiments/v11_runpod_pythia_moe/results/v11_olmoe_1b_7b_results.json`.

- **ImageNet ViT-L/16 pretrained (production-scale ViT, multi-seed)**: Torchvision's `ViT_L_16_Weights.IMAGENET1K_SWAG_LINEAR_V1` (304.3 M parameters, 24-layer Vision Transformer with 16×16 patches and 1024-dim hidden, 79.66 % ImageNet-1K top-1). Same protocol as V9.5/V9.6 (200 probes, float64), 5 probe seeds. Multi-seed result: $\log_{10}(T_1/T_3) = 3.85 \pm 0.002$ (95 % bootstrap CI $[3.84, 3.85]$), $T_1/T_3 \approx 7\,000$ (deep-sequential band, $69 \times$ above the 100 threshold), Gini = $0.894 \pm 0.000$, top-1 % mass = $0.569 \pm 0.001$. ViT-L/16 has the *tightest* multi-seed CI of all three production-scale models — the production-scale FIM-diagonal estimator is essentially deterministic at 200 probes for this architecture. Combined with the ResNet-50 V1+V2 results (CNN) and GPT-2-medium (autoregressive-LM, single-seed below; multi-seed for GPT-2-medium ran into per-seed VRAM constraints on RTX 3080 and is reported as single-seed point estimate with the V6.0b probe-convergence study standing in for variance characterisation), this gives **production-scale data points across CNN + ViT + autoregressive-LM** at $\geq 25$ M parameters. Full results: `experiments/v9_modern_arch/v9_7_imagenet_vit_l16_results.json` and `experiments/v9_modern_arch/v9_multi_seed_production_results.json`.
...
| **ResNet-50 (V1, 5-seed)** | **25.6 M** | **image (CNN)** | **ImageNet (76.13 %)** | $\mathbf{10^{27.4 \pm 0.2}}$ † | **0.99** | **0.76** | **yes** |
| **ResNet-50 (V2)** | **25.6 M** | **image (CNN)** | **ImageNet (80.86 %)** | $\mathbf{7.32 \times 10^{6}}$ | **1.00** | **0.96** | **yes** |
| **ViT-B/16 (random init)** | **85.8 M** | **image (attn)** | **(none — from-scratch trajectory)** | $\mathbf{0.37}$ | 1.00 | 0.26 | **NO** (only sub-100 substrate at random init) |
| **ViT-B/16 (epoch 10)** | **85.8 M** | **image (attn)** | **Imagenette (10 ep, 71.5 %)** | $\mathbf{2.9 \times 10^{4}}$ | 0.96 | 0.70 | **yes** |
| **ViT-L/16** | **304.3 M** | **image (attn)** | **ImageNet (79.66 %)** | $\mathbf{6.93 \times 10^{3}}$ | **0.89** | **0.57** | **yes** |
| **GPT-2-medium (multi-seed)** | **354.8 M** | **language** | **WebText (40 GB)** | $\mathbf{6.7 \times 10^4}$ ± 0.6 % | **1.00** | **0.99** | **yes** |
| **GPT-2-large** | **774 M** | **language** | **WebText (40 GB)** | $\mathbf{1.1 \times 10^3}$ | **0.87** | **0.61** | **yes** |
| **Pythia-1.4B** | **1.4 B** | **language** | **The Pile** | $\mathbf{4.25 \times 10^3}$ | **0.94** | **0.80** | **yes** |
| **Pythia-2.8B** | **2.78 B** | **language** | **The Pile** | $\mathbf{2.02 \times 10^3}$ | **0.91** | **0.75** | **yes** |
| **Pythia-6.9B (V11.6)** | **6.86 B** | **language (dense)** | **The Pile** | $\mathbf{5.74 \times 10^3}$ | **0.96** | **0.87** | **yes** |
| **OLMoE-1B-7B (V11.6)** | **6.92 B (1 B active)** | **language (sparse-MoE)** | **The Pile / Dolma** | $\mathbf{4.26 \times 10^5}$ | **1.00** | **0.98** | **yes** ($\sim 74\times$ over dense at matched total scale) |

† **Footnote on ResNet-50 V1 magnitude.** The 5-seed multi-seed mean is $10^{27.4 \pm 0.2}$, with per-seed values spanning $1.5 \times 10^{27}$ to $3.3 \times 10^{27}$. The extreme magnitude is driven by an effectively-zero bottom 50 % (T₃ at fp64 underflow, $\approx 10^{-21}$) — a consequence of dead-ReLU activations whose gradients zero out the lower-tier FIM diagonals. The dichotomy *direction* is unaffected; the partition-free statistics (Gini = 0.99, top-1 % mass = 0.76) are robust. Reviewers should read this row's $T_1/T_3$ as "extreme heavy-tail consistent with dead-ReLU floor at fp64 precision," not as a stable point estimate.
...
**Pretrained-checkpoint provenance.** All production-scale results use the canonical published torchvision / HuggingFace pretrained weights, downloaded by the script `experiments/v9_modern_arch/imagenet_resnet50_fim.py` (and analogous files) at runtime. Exact identifiers and SHA-256 of the downloaded `.pth` / `.safetensors` files are pinned in `docs/claim_manifest.json`:

| Model | Source | Weights identifier | Top-1 acc | SHA-256 of weights file |
|---|---|---|---|---|
| ResNet-50 V1 | torchvision $\geq$ 0.13 | `ResNet50_Weights.IMAGENET1K_V1` | 76.13 % | `0676ba6...` (90-epoch baseline) |
| ResNet-50 V2 | torchvision $\geq$ 0.13 | `ResNet50_Weights.IMAGENET1K_V2` | 80.86 % | `11ad3fa6...` (modern recipe) |
| ViT-L/16 | torchvision $\geq$ 0.15 | `ViT_L_16_Weights.IMAGENET1K_SWAG_LINEAR_V1` | 79.66 % | `4d563306...` (SWAG features + linear classifier on ImageNet-1K) |
| GPT-2-medium | HuggingFace `transformers` | `gpt2-medium` (Radford et al. 2019, OpenAI Tech Report) | n/a (autoregressive LM) | sharded; see `~/.cache/huggingface/hub/models--gpt2-medium/blobs/` |

We do *not* access raw ImageNet or WebText data; FIM-diagonal probes are drawn from ImageNet-statistics-normalised Gaussian inputs (vision) and from a fixed 12-sentence English text sample (language). The pretrained weights themselves are the only product of the original training data that enters our analysis. This addresses the broader-impact concern of data-provenance uncertainty (§7).

**Asymptotic-family context.** The functional form $\log(T_1/T_3) = c\,\sigma\,\sqrt{L} + o(\sqrt{L})$ places the FIM tier ratio in the same broad subexponential-in-$\sqrt{\text{scale}}$ asymptotic family as several classical results in additive combinatorics — most prominently the Hardy–Ramanujan partition asymptotic $p(n) \sim (4n\sqrt{3})^{-1} \exp\!\bigl(\pi \sqrt{2n/3}\bigr)$ \cite{hardy1918asymptotic}. We claim a *shape neighbourhood*, not a derivation: the $\sqrt{L}$ growth in our setting comes from log-normal quantile algebra applied to Hanin–Nica's depth-linear log-FIM variance (Theorem 1' + Corollary 1, Appendix B), not from any partition-counting or generating-function identity, and the underlying mechanisms (saddle-point on the modular form $\eta(\tau)^{-1}$ vs. truncated-Gaussian moment generating functions) are unrelated. The shared form is suggestive of a wider "deep, layered, structurally-constrained accumulation" universality for which no single proof framework yet covers both random-Jacobian product spectra and integer-partition counts. We flag the neighbourhood as positioning context — it answers a likely reviewer question of "why $\sqrt{L}$ specifically?" by noting that this exponent is shared with one of the oldest deep-counting asymptotics in mathematics — but is not load-bearing for any claim in this paper.
...
- **Parameter scale.** Architectures explored span $10^3$–$10^9$ parameters. Extrapolation beyond this measured range is conjectural; we make no quantitative claim outside it.
- **Production-scale architectures.** V9–V11 measure the dichotomy on ResNet-50 V1+V2 (25.6 M), ViT-L/16 (304.3 M), ViT-B/16-trajectory-from-scratch (85.8 M), GPT-2-medium 5-seed multi-seed (354.8 M), GPT-2-large (774 M), Pythia-1.4B (1.4 B), and an ImageNet-1K ResNet-50 from-scratch trajectory at 90-epoch full scale; all measured production-scale checkpoints sit firmly in the deep-sequential band ($T_1/T_3 \geq 11 \times$ the 100 threshold). The remaining production-scale experiment we have not measured is ImageNet ViT-Large (304 M) trained *from scratch*, which requires ~120 H100-GPU-hours at the canonical 90-epoch recipe; we have its mechanism-prediction pre-registered in `docs/preregistration_v2.md`, but the spend-versus-evidential-marginal-utility tradeoff means this experiment is held until a reviewer requests it. The `docs/h200_cluster_runbook.md` runbook reproduces all of the V11 results on H200 hardware in $\leq$ 2 GPU-hours.
...
A large-$N$ tier-fraction theorem (formal proof that the limit fractions $(0.01, 0.49, 0.50)$ are well-defined under a specified parameter-prior class) and a tighter mechanism for residual + attention architectures are left open for future work. **Three distinct from-scratch trajectory patterns are documented in §4.6**, with a hierarchy: (A) CIFAR-10 ResNet-18 monotonic *decrease* — also confirmed at canonical-full-ImageNet-1K-90-epoch scale on ResNet-50 (V11.4, $T_1/T_3$ drops 2 300× by epoch 30); (B) Imagenette ResNet-50 *increase* with epoch-1 transient peak — now re-framed as a small-data-regime outlier (10-class, 13 k images), since the same ResNet-50 architecture follows Pattern A at full ImageNet-1K scale; (C) Imagenette ViT-B/16 jump-from-below-threshold (random init $T_1/T_3 = 0.37$, epoch 1 jumps to $> 10^5$, epoch 10 settles at $2.9 \times 10^4$). The dichotomy magnitude is preserved across all three patterns at every trained checkpoint; trajectory *direction* is architecture-and-dataset-specific. Cosmological framings of the deep-layered-sequential class (Vanchurin 2020; Anonymous 2026) are out of scope for this paper and discussed only briefly in Appendix C.
...
**Checkpoint provenance and data exposure.** Pretrained-weight production-scale measurements (§4.6) use the canonical *published* weights from torchvision and HuggingFace (ResNet-50 V1+V2, ViT-L/16 SWAG-LINEAR, GPT-2-medium, GPT-2-large, Pythia-1.4B); from-scratch production-scale measurements (V9.5b Imagenette ResNet-50, V9.8b Imagenette ViT-B/16, V11.3 ImageNet-1K ResNet-50) use random initialisation and explicit gradient-descent training under the protocol pre-registered in `docs/preregistration_v2.md`. For pretrained checkpoints, we do **not** access the raw ImageNet-1K, WebText, or Pile training data; the FIM-diagonal probes are drawn from synthetic ImageNet-statistics-normalised Gaussian inputs (vision) and a fixed 12-sentence English text sample (language). For from-scratch measurements, we use the standard published Imagenette and ImageNet-1K splits; our exposure is identical to that of any ImageNet-trained model and we do not redistribute the underlying images. The published or freshly-trained weights are the only product of training data that enters our analysis, and we do not amplify any data-provenance concerns documented for the upstream training corpora.

**Environmental footprint.** All small-scale panel experiments (V1–V8 plus V10/V10b, ~15 individual sweeps) ran on a single RTX 3080 at consumer-residential power; total wall time $\leq$ 4 GPU-hours. Production-scale FIM measurements (V9.5 ResNet-50 V1 + V2, V9.6 GPT-2-medium, V9.7 ViT-L/16, plus 5-seed multi-seed for ResNet-50 V1+V2 + ViT-L/16) ran in $\leq$ 90 GPU-minutes total on the same single RTX 3080. The cluster A100 component (V3.0 cluster scaling to $N = 1.45 \times 10^9$) consumed $\leq$ 3.5 GPU-hours on a community-cloud A100 80 GB at $\sim\$5$ USD. Total estimated emissions across the entire paper: $\leq$ 10 kgCO₂-eq using a conservative 250 W average draw and the European grid emission factor (0.27 kgCO₂/kWh). The pretrained-checkpoint cost we *use* is not the cost we *produced* — the original ResNet-50 / ViT-L/16 / GPT-2-medium training runs are well-known to be much larger ($\sim$ 100 GPU-days for ImageNet ResNet-50 V2 with the modern recipe; $\sim$ 1 200 GPU-days for GPT-2-medium); we contribute zero new pre-training compute.

**Misuse mitigation.** The diagnostic should not be used as the *sole* criterion for architecture selection or pruning decisions without downstream validation. Specifically: (a) a high $T_1/T_3$ at random initialisation does *not* imply that the network is well-trained or that the top-1 % parameters identified are the correct candidates for FIM-saliency-based pruning (cf. SNIP, GraSP, Synaptic Flow literature, which use related but distinct saliency definitions and report mixed pruning success); (b) a low $T_1/T_3$ does not imply that the architecture is *bad* — RNN/LSTM, attention with tied embeddings, and selective-gating SSMs all give relatively low $T_1/T_3$ on small-scale runs but are the foundation of much of contemporary applied deep learning; (c) the dichotomy threshold of 100 is calibrated on the V2 panel and may not transfer directly to architectures outside that panel. We recommend the diagnostic be used as one signal in an ensemble with downstream task metrics, gradient stability checks, and direct ablations on the candidate architecture, *not* as a standalone reject/accept gate.
...
[1] Anonymous. "The Universe as a Self-Organizing Neural Network." Anonymous, April 2026.
