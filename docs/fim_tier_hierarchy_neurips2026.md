...
**Headline contribution: a new computational-substrate universality class.** We define and empirically establish *deep layered sequential composition* as a non-trivial universality class that the Fisher Information diagonal observable detects. To our knowledge, this is the first formal substrate-class universality result in deep-network observables since the NTK-vs-feature-learning dichotomy (Yang & Hu, ICML 2021), and the first to extend a deep-learning observable to non-neural substrates (random boolean circuits, MERA-style tensor networks) under a single statistical protocol.

We make six concrete contributions, none of which (to our knowledge) appear in prior work:

1. **A new universality class.** Prior work characterised heavy-tailed FIM spectra in MLPs (Karakida–Akaho–Amari, AISTATS 2019) and three-level outlier structure in deepnet Hessians (Papyan, ICML 2019), both restricted to neural networks. We show empirically that the same diagonal hierarchy is a property of *layered recursive composition* as a computational primitive — present in boolean circuits ($T_1/T_3 \sim 10^7$, no learning, no gradients), balanced binary tensor networks, BatchNorm ResNets, vanilla transformers, and trained / untrained MLPs; absent from shallow parameterised learners that *do* learn ($T_1/T_3 \le 5$), lattice gauge fields, dynamical systems, and matrix ensembles ($T_1/T_3 \le 80$). The 12-substrate panel achieves *complete rank separation* with $p = 1.7 \times 10^{-17}$, $r = 1.000$, ROC AUC $= 1.0$. The substrate-class itself is the load-bearing novelty.
...
6. **Production-scale verification at $\geq 1$B parameters.** Multi-seed FIM measurements on ResNet-50 V1+V2, ViT-L/16 (304 M), GPT-2-medium (5-seed multi-seed, 354.8 M), GPT-2-large (774 M), Pythia-1.4B (first billion-parameter LM data point), and Pythia-2.8B (V11.3 second billion-parameter LM). Plus an Imagenette ViT-B/16 from-scratch trajectory documenting a third distinct trajectory pattern. Every measured trained checkpoint sits firmly above the 100 threshold; the dichotomy is preserved at production scale.

The mechanism's connection to Hanin–Nica's product-of-random-matrices theorem is an *application* of an existing result, not a new theorem; we make this honest scope statement explicit. Contributions 1, 3, 4, 5, 6 are empirical and methodological; contribution 2 is a formal identity that, while elementary in its proof, is the first to bridge log-normal random-matrix theory to substrate-class universality in deep networks.
