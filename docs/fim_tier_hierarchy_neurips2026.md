...
| 11 | The substrate class extends beyond our 12-class panel (e.g. to MoE, MLP-Mixer, SSM variants we have not tested) | **Empirically supported for MoE (V11.6)**: OLMoE-1B-7B-0924 measured at $T_1/T_3 = 4.26 \times 10^5$ vs dense Pythia-6.9B at $5.74 \times 10^3$ — ~74× more concentration at matched ~7 B total-parameter scale. MLP-Mixer, SSM (Mamba) variants beyond V9.4 still **conjectural** (open) | V11.6 OLMoE results JSON; MLP-Mixer + SSM extensions require out-of-panel measurement |
...
6. **Production-scale verification through 2.8 B parameters and a complete from-scratch ImageNet-1K trajectory.** Multi-seed FIM on ResNet-50 V1+V2, ViT-L/16 (304 M), GPT-2-medium 5-seed multi-seed (354.8 M, $\log_{10} = 4.83 \pm 0.003$), GPT-2-large (774 M), Pythia-1.4B and Pythia-2.8B (V11.3 — *first* and *second* billion-parameter LM data points in the FIM-diagonal literature). The V11.5 capstone is a full **ImageNet-1K ResNet-50 90-epoch from-scratch trajectory** at the canonical NeurIPS-era recipe (74.6 % top-1, $T_1/T_3$ drops $4\,367\times$ monotonically) — Pattern A confirmed at canonical scale, *re-framing* the V9.5b Imagenette anomaly as a small-data-regime outlier rather than an architectural property of ResNet-50. Plus three distinct from-scratch trajectory patterns documented (CIFAR-10 ResNet-18 monotonic decrease; Imagenette ResNet-50 increase + transient peak; Imagenette ViT-B/16 jump-from-below-threshold). Every trained-equilibrium checkpoint sits firmly in the deep-sequential band.
